...
for (i in unique_endpoints) {
  meta_result_list[[i]] <- meta::metagen(
    ...
    data = meta_df_final %>% filter(outcome == i) %>%
      mutate(
        `Target Pathogen` = factor(`Target Pathogen`,
          levels = c("CRAB", "MDR")
        ),
        `Target Pathogen` = fct_recode(`Target Pathogen`,
          "CRAB infections only" = "CRAB",
          "Mixed infections" = "MDR"
        )
      ), , # ensure that CRAB group is always displayed first
    sm = "OR",
    ...
    subgroup = `Target Pathogen`,
    print.subgroup.name = FALSE
  )
  png(file = paste0("Plots/forestplot_", i, ".png"), width = 3500, height = 1700, res = 300)
